...
class FeatureEngineer:
    ...
    def _create_scale_features(self, df: pd.DataFrame, fit: bool) -> pd.DataFrame:
        ...
        - interventions_per_arm: Intervention density per arm
        - conditions_per_arm: Condition heterogeneity per arm
        - enrollment_per_arm: Enrollment per arm
        - enrollment_met: Enrollment met
        - complexity_per_enrollment: Complexity normalized by log enrollment
        - duration_per_enrollment: Study duration normalized by log enrollment
        """
        result = df.copy()
        ...
        if 'enrollment_count' in df.columns:
            ...
            result['enrollment_count'] = result['enrollment_count'].clip(
                upper=self.params['enrollment_q99']
            )
        ...
        if 'num_interventions' in result.columns and 'estimated_arms' in result.columns:
            result['interventions_per_arm'] = (
                result['num_interventions'] / result['estimated_arms']
            ).clip(lower=0.5)
        
        if 'num_conditions' in result.columns and 'estimated_arms' in result.columns:
            result['conditions_per_arm'] = (
                result['num_conditions'] / result['estimated_arms']
            ).clip(lower=0.5)
        ...
        if 'total_complexity_score' in result.columns and 'enrollment_count' in result.columns:
            denom = np.log1p(result['enrollment_count']).clip(lower=1)
            result['complexity_per_enrollment'] = (
                result['total_complexity_score'] / denom
            )
            
        if 'study_duration_months' in result.columns and 'enrollment_count' in result.columns:
            denom = np.log1p(result['enrollment_count']).clip(lower=1)
            result['duration_per_enrollment'] = (
                result['study_duration_months'] / denom
            )
    ...
    def get_feature_list(self) -> List[str]:
        ...
        return [
            ...
            'total_complexity_score', 'complexity_score_log', 'complexity_per_enrollment', 
            'duration_per_enrollment', 'interventions_per_arm', 'conditions_per_arm',
            # Scale
